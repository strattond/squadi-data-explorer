...
def getMatchingUserMatch( userDivData: fixed.DivisionData | user.DivisionData | None, match ):
  if userDivData is None:
    return None
  return next( ( uMatch for uMatch in userDivData.matches if uMatch.match.id == match ), None )


def getMatchingPlayer( name, userMatchNum: fixed.FixtureWrapper | user.FixtureWrapper | None ):
